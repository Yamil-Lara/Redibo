...
// Confirmar pago
export const confirmarPago = async (idReserva: number) => {
  try {
    // Buscar la reserva
    ...
    if (reserva.estado !== 'APROBADA') {
      return {
        ok: false,
        status: 400,
        message: `La reserva debe estar en estado APROBADA para ser confirmada. Estado actual: ${reserva.estado}`
      };
    }
    ...
    console.log(`✅ Reserva #${idReserva} actualizada a estado CONFIRMADA.`);
    return {
      ok: true,
      status: 200,
    ...
    return { ok: false, status: 500, message: 'Error interno al confirmar la reserva' };
  }
};
