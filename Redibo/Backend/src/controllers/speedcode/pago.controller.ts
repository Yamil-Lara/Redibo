...
import * as PagoService from '../../services/speedcode/pago.service';
import { sendEmail } from '../../utils/mailer';
import { generarImagenPago } from '../../utils/generarImagen';
import { MetodoPago } from '@prisma/client';
import { validarTarjeta } from '../../middlewares/pagos/validarTarjeta'
import { validarQR } from '../../middlewares/pagos/validarQR'
import { confirmarPago } from './../speedcode/reservas.controller';
...
export const registrarPago = async (
  ...
  try {
    ...
    if (!nuevoPago) return { error: 'No se pudo registrar el pago.' };

    const resultadoConfirmacion = await confirmarPago(reserva_idreserva);
    if (!resultadoConfirmacion.ok) {
      console.warn(`⚠️ El pago se registró, pero no se pudo confirmar la reserva: ${resultadoConfirmacion.message}`);
    }
